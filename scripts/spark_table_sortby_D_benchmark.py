...
def run_benchmarks(base_path):
    ...
    print("Time taken for sort on D:")
    start_task=time.time()

    ## sort on D
    df = hive_context.sql('SELECT * FROM temp ORDER BY d')
    print(df.count())
    end_task=time.time()
    end=time.time()
...
def main():
    ...
    df_op.coalesce(1).write.format('com.databricks.spark.csv').option("header","true").save("s3://bucket-name/folder-name/op_table_sortby_d")
