...
def run_benchmarks(base_path):
    ...
    print("Time taken for sort on C:")
    start_task=time.time()

    ## sort on C
    #sorted_df = df.sort(df.c.desc())
    df = hive_context.sql('SELECT * FROM temp ORDER BY c')
    print(df.count())
...
def main():
    ...
    df_op.coalesce(1).write.format('com.databricks.spark.csv').option("header","true").save("s3://bucket-name/folder-name/op_table_sortby_c")
