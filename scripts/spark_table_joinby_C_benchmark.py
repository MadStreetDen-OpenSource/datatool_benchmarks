...
def run_benchmarks(base_path,base_path2):
    ...
    joined_df = hive_context.sql('SELECT * FROM temp1 JOIN temp2 ON (temp1.c = temp2.c)')
...
def main():
    ...
    df_op.coalesce(1).write.format('com.databricks.spark.csv').option("header","true").save("s3://bucket-name/folder-name/op_table_joinby_c")
