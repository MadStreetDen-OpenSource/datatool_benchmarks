...
def run_benchmarks(base_path):
    ...
    print("Time taken for groupby c and sum on ID:")
    start_task=time.time()

    ## groupby
    #grouped_df = df.groupBy('c').sum('id')
    grouped_df = hive_context.sql('SELECT c, COUNT(id) FROM temp GROUP BY c')
...
def main():
    ...
    df_op.coalesce(1).write.format('com.databricks.spark.csv').option("header","true").save("s3://bucket-name/folder-name/op_table_groupby_c")
