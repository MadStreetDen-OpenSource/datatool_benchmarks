...
def run_benchmarks(base_path):
    ...
    rdd_sortby_C=rdd.map(lambda x: (x[3], x))
    #print(rdd_groupby_C.take(20))
    rdd_sorted_by_C=rdd_sortby_C.sortByKey()
    #print(rdd_grouped_C.take(20))
    print(rdd_sorted_by_C.count())
    end_task=time.time()
    end=time.time()
    #print(end-start)
...
def main():    
    ...
    df_op.coalesce(1).write.format('com.databricks.spark.csv').option("header","true").save("s3://bucket-name/folder-name/op_rdd_sortby_D")
