...
def run_benchmarks(base_path):
    ...
    print("Time taken for groupBy on DataFrame column D followed by sum aggregate: ")
    start_task=time.time()
    df_groupby_D=df.groupBy('D').agg(F.sum(df.id))
    print(df_groupby_D.count())
    end_task=time.time()
    end=time.time()
...
def main():    
    ...
    df_op.coalesce(1).write.format('com.databricks.spark.csv').option("header","true").save("s3://bucket-name/folder-name/op_df_groupby_D")
