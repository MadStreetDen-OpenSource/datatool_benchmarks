...
def benchmark(files,csv_filename):
   with open(csv_filename,"w") as f:
       ...
       csv_writer.writerow(["PD_string","PD_num","NP"])
       for i in xrange(7):
           print files[i]
           setup_pd_copy = setup_pd+';i='+str(i)
           setup_np_copy = setup_np+';i='+str(i)
          
           setup_pd_copy = setup_pd_copy+"; p = pd.read_csv(files[i], names=('A','B','C','D','E'))"
           setup_np_copy = setup_np_copy+"; n=np.load('"+files[i].split(".csv")[0]+"_only_num.npz')['arr_0']; "
           pd_time_elapsed_string = timeit("p.groupby(['D'],as_index=False)['A'].sum()",setup=setup_pd_copy,number=3)
           pd_time_elapsed_int = timeit("p.groupby(['E'],as_index=False)['A'].sum()",setup=setup_pd_copy,number=3)
           np_time_elapsed = timeit("for i in groupby(n,itemgetter(1)): x=sum((m[0] for m in i[1] ))",setup=setup_np_copy,number=3)
...
benchmark(files_list,"../data/aggregate.csv")
